...
# libstlport
# M removes libstlport, but some of our binary-only prebuilts need it, so we'll add it back
PRODUCT_PACKAGES += \
    libstlport
...
PRODUCT_COPY_FILES += \
    ...
    $(COMMON_PATH)/configs/keylayout/gpio_keys_8.kl:system/usr/keylayout/gpio_keys_8.kl \
...
PRODUCT_COPY_FILES += \
    ...
    $(COMMON_PATH)/configs/nfc/libnfc-brcm.conf:system/etc/libnfc-brcm.conf \
    $(COMMON_PATH)/configs/nfc/libnfc-sec.conf:system/etc/libnfc-sec.conf \
    $(COMMON_PATH)/configs/nfc/libnfc-nxp.conf:system/etc/libnfc-nxp.conf
